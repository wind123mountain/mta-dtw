...
def evaluate_main(args, tokenizer, model, dataset: PromptDataset, split, epoch, device):
    ...
    dataset_tag = str(args.data_names).replace("\\", "_").replace("/", "_").replace(os.sep, "_").replace(" ", "_")
    seed_tag = str(args.seed)
    
    # Write predictions to dataset- and seed-specific preds file; only rank 0 writes
    if (not dist.is_initialized()) or dist.get_rank() == 0:
        preds_txt_path = os.path.join(args.save_dir, f"preds.{dataset_tag}.{seed_tag}.txt")
        with open(preds_txt_path, "w", encoding="utf-8") as f:
    ...
    if (not dist.is_initialized()) or dist.get_rank() == 0:
        torch.save(all_preds, os.path.join(args.save_dir, f"preds.{dataset_tag}.{seed_tag}.pt"))

    all_responses = []
    answers_jsonl_path = os.path.join(args.save_dir, f"answers.{dataset_tag}.{seed_tag}.jsonl")
    if (not dist.is_initialized()) or dist.get_rank() == 0:
        with open(answers_jsonl_path, "w", encoding="utf-8") as f:
    ...
    print_rank(log_str)
    # Append unified log under .../eval/log.txt regardless of dataset/seed
    try:
        cur = os.path.abspath(args.save_dir)
        while True:
            parent = os.path.dirname(cur)
            if os.path.basename(parent) == "eval" or parent == cur:
                eval_root = parent if os.path.basename(parent) == "eval" else os.path.abspath(args.save_dir)
                break
            cur = parent
        save_rank(log_str, os.path.join(eval_root, "log.txt"))
    except Exception:
        # Fallback to local log.txt
