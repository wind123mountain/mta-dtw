...
SETTING=criterion=${CRITERION}+dtw__epoch=${EPOCH}__teacher=${TEACHER_MODEL_TYPE}__kd^rate=${KD_RATE}__dtw^rate=${DTW_RATE}__bsz=${BATCH_SIZE}x${GRAD_ACC}x${GPUS_PER_NODE}__lr=${LR}
...
# OPTS+=" --wandb --wandb-project OpenMath-Distill --wandb-run-name gemma3_270m_dskd_dtw_full"
