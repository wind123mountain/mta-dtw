...
SETTING=criterion=${CRITERION}__epoch=${EPOCH}__teacher=${TEACHER_MODEL_TYPE}__kd^rate=${KD_RATE}__ce^rate=${CE_RATE}__bsz=${BATCH_SIZE}x${GRAD_ACC}x${GPUS_PER_NODE}__lr=${LR}
...
# OPTS+=" --wandb --wandb-project OpenMath-Distill --wandb-run-name gemma3_dskd"
